...
if (!empty($_GET['q'])) require_once('sphinxapi.php');
                        ...
                        <div class="pagecontent">
                            <div class="dropCase">
                              <table width="100%">
                                ...
                                <tr>
                                  <td valign="top">
                                    <div style="padding: 0; margin: 0">Keywords: <?php foreach($keys as $i=>$wrd){ echo htmlentities($wrd).", "; } ?></div>
                                  </td>
                                </tr>
...
if ($num_res1 > 0) {
    foreach ($result1['matches'] as $id => $other) {
        $data = $other['attrs'];
        $ids1 .= "assession_number=\"" . mysql_real_escape_string($data['assession_number']) ."\" OR ";
    }
}
...
if ($num_res2 > 0) {
    foreach ($result2['matches'] as $id => $other) {
        $data = $other['attrs'];
        $ids2 .= "fold_type=\"" . mysql_real_escape_string($data['fold_type']) ."\" OR ";
    }
}
